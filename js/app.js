	...
	if (storageData != null) {
		...
		data.forEach(column => {
			...
			column.cards.forEach(card => {
				document.querySelector('#' + column.id).appendChild(createCard(card.text));
				cardCounter++;
			});
		});
	}
...
function createCardInput() {
	...
	let cardInput = createElement('input', { type: 'text', placeholder: 'Enter text for this card' });
...
function createCard(text) {
	return createElement('p', { textContent: text, className: 'card', id: 'card-' + cardCounter });
}
